...
omega_count= 5            #number of omegas to try from pi/(omega_count/2) to 2*pi
inc_count= 5              #number of inclinations -1 to try from -pi/2 to pi/2 (we have 30 steps)
...
res = {}
with open('/Users/Behzadarbab/Exoplanet_Simulations/Earth_Omega&inc_Var/new/chances_x({min}-{max})_{total_count}k.txt'.format(min=min(x_list),
                                                                                                                       max=max(x_list),
                                                                                                                       total_count=omega_count*(inc_count)/1000), 'w+') as g:
    g.write('x\t%\n')
    with open('/Users/Behzadarbab/Exoplanet_Simulations/Earth_Omega&inc_Var/new/Data_x({min}-{max})_{total_count}k.txt'.format(min=min(x_list),
                                                                                                                       max=max(x_list),
       ...
       for m in m_list:
            ...
            for x in x_list:
                ...
                for v in vz_list:
                    ...
                    plt.savefig('/Users/Behzadarbab/Exoplanet_Simulations/Earth_Omega&inc_Var/new/X{x}m{m}v{v}({total_count}k)({P} percent).pdf'.format(x=round(x,1),m=m, v=v, total_count=(omega_count*(inc_count)/1000),

    ...
    for t in range(1,26):
        ...
        for m in m_list:
            ...
            for x in x_list:
                ...
                for v in vz_list:
                    ...
                    for inc in inc_list:
                        ...
                        for o in omega_list:
                            ...
                            sim.add(m=m, x=x, z=20*t, vz=v)                          #add the passing star

                            sim.integrate(round(2 * 20*t / (-v), 0))                           #integration
                            ...
                            plt.plot(o,inc,color=Color,marker='.',ms=0.3)
                ...
                plt.xlabel('Omega (initial phase)', fontsize='3')
                plt.ylabel('inclination', fontsize='3')
                plt.title('t={t}, X={x}, ({total_count}k)({P} percent)'.format(t=t, x=round(x,1), total_count=(omega_count*(inc_count)/1000), P=round(Percentage,2)), fontsize='3')
                # plt.axes().set_aspect('equal', 'datalim')


plt.savefig('/Users/Behzadarbab/Exoplanet_Simulations/Earth_Omega&inc_Var/X{x}m{m}v{v}({total_count}k)_timetest.pdf'.format(x=round(x,1),m=m, v=v, total_count=(omega_count*(inc_count)/1000),
                                                                                                                                    ))

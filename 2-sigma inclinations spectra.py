...
res = {}
with open('/Users/atefeh-behzad/Exoplanet_Simulations/2-Sigma/inclination2_v({v:.2f})_m({m:.2f})_x({xmin:.2f}_{xmax:.2f}).txt'.format(v=v, m=m, xmin=x_min, xmax=x_max) , 'w+') as g:
    g.write('x\t')

    '''here we make lists of omega,x,vz,inc and m'''
    omega_list = get_list(0, 2 * math.pi / omega_count, 2 * math.pi - math.pi / omega_count,
                          rel=operator.le)
    inc_list = get_list(r=0, s=math.pi / (inc_count),
    ...
    for inc in inc_list:
        g.write('{inc:.3f}\t'.format(inc=inc))
    g.write('\n')
    for x in x_list:
        g.write('{x:.2f}\t'.format(x=x))
        ...
        for inc in inc_list:
            ...
            g.write('{percent:.4f}\t'.format(inc=inc, x=x, percent=Percentage))
        g.write('\n')

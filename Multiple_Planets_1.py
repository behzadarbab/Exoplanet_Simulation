...
res = {}
with open('/Users/atefeh-behzad/Exoplanet_Simulations/Earth_Omega_inc_f_4/chances_incchange_x({min:.2f}-{max:.2f})_{total_count}k.txt'.format(min=min(x_list),
                                                                                                                       max=max(x_list),
                                                                                                                       total_count=omega_count*(inc_count)/1000), 'w+') as g:
    g.write('x\t%\tinc\tincdifavg\n')
    with open('/Users/atefeh-behzad/Exoplanet_Simulations/Earth_Omega_inc_f_4/Data_x({min:.2f}-{max:.2f})_{total_count}k.txt'.format(min=min(x_list),
                                                                                                                       max=max(x_list),
        ...
        for m in m_list:
            ...
            for x in x_list:
                ...
                for v in vz_list:
                    ...
                    for inc in inc_list:
                        ...
                        for o in omega_list:
                            ...
                            for f in f_list:
                                ...
                                sim.add(m=1e-3, a=5.2)
                                sim.add(m=2.8e-4, a=9.55)
                    ...
                    plt.savefig('/Users/atefeh-behzad/Exoplanet_Simulations/Earth_Omega_inc_f_4/X{x}m{m}v{v}({total_count}k).pdf'.format(x=round(x,2),m=m, v=v, total_count=(omega_count*(inc_count)*f_count/1000)
                                                                                                                                        ), bbox_inches='tight')

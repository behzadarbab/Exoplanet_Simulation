...
res = {}
with open('/Users/atefeh-behzad/Exoplanet_Simulations/2-Sigma/HZhw({hw:.2f})_v({minv:.2f}-{maxv:.2f})_m({m:.2f}).txt'.format(hw=0.2, minv=min(vz_list), maxv=max(vz_list), m=m) , 'w+') as g:
    g.write('m\tv\tx\n')
    for v in vz_list:
        flag = 1
        x_min=x_min0
